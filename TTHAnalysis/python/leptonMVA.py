...
_CommonVars = {
 'forMoriond':[ 
    ...
    MVAVar("LepGood_jetNDauChargedMVASel",lambda lepton: sum((deltaR(x.eta(),x.phi(),lepton.jet.eta(),lepton.jet.phi())<=0.4 and x.charge()!=0 and x.fromPV()>1 and (qualityTrk(x.pseudoTrack(),lepton.associatedVertex) if x.hasTrackDetails() else False)) for x in lepton.jet.daughterPtrVector()) if hasattr(lepton,'jet') and lepton.jet != lepton else 0),
